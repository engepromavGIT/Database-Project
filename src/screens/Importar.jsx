...
export function Importar() {
  ...
  const confirmar = async () => {
    ...
    try { setResultado(await api.importarConfirmar(analise.linhas, analise.mapa)) }
    ...
    <div style={{ display: 'grid', gap: 'var(--sp-4)' }}>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead><tr style={{ textAlign: 'left', color: 'var(--fg-3)' }}>
                <th>Código</th><th>Nome</th><th>Tipo</th><th>Área</th><th>Custo real</th><th>UF</th><th>Início</th><th>Fim</th><th>Ref.</th>
              </tr></thead>
              <tbody>
                {analise.previa.map((l, i) => (
                  <tr key={i} style={{ borderTop: '1px solid var(--border)' }}>
                    <td>{l.codigo || '—'}</td><td>{l.nome || '—'}</td><td>{l.tipoNome || '—'}</td>
      ...
      {resultado && (
        <section className="card" style={{ padding: 'var(--sp-4)' }}>
          ...
          <p><strong>{resultado.inseridas}</strong> de {resultado.total} obras importadas.</p>
